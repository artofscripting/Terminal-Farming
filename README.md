...
| `Z` | Auto-play — runs your farm hands-off, one action every 0.1s: harvests ripe crops, waters, plants, tills bare owned ground, buys more seed once it runs out (picking whatever crop profits best and still safely matures before the next frost risk), and sleeps when there's nothing left to do; press `Z` again to cancel |
